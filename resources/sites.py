...
class SitesHelper:
    ...
    def __init__(self):
        ...
        self.auth_level = 2
        self.auth_version = '99.10'
        self.indexer_version = '23.10'
        ...
        self.sites_indexer = {
            'm-team': {
                ...
                'ext_domains':
                ['https://xp.m-team.cc/', 'https://kp.m-team.cc/'],
                'encoding': 'UTF-8',
                'parser': 'mTorrent',
                'schema': 'MTorrent',
                'ua':
                ...
                'torrents': {
                    'list': {
                        'selector':
                        'table.torrents > tr:has("table.torrentname")'
                    },
                    'fields': {
                        'id': {
                        ...
                        'date_added': {
                            'selector':
                            'td.rowfollow:nth-last-child(7) > span',
                            'attribute': 'title',
                            'optional': True
                        },
                        'date_elapsed': {
                        ...
                        'labels': {
                            'selector':
                            'table.torrentname > tr > td.embedded > img[class*="label_"]',
                            'attribute': 'alt'
                        },
                        'hr': {
            ...
            'ptchdbits': {
                ...
                'torrents': {
                    ...
                    'fields': {
                        ...
                        'title_default': {
                            'selector':
                            'table.torrentname > tr > td.embedded > a[href*="details.php?id="]'
                        },
                        'title_optional': {
                            'optional': True,
                            'selector':
                            'table.torrentname > tr > td.embedded > a[title][href*="details.php?id="]',
                            'attribute': 'title'
                        },
                        'title': {
                            'text':
                        ...
                        'minimumratio': {
                            'text':
                            "{% if fields[ 'hr_days' ] %}999999{% else %}0{% endif %}"
                        },
                        'minimumseedtime': {
                            'text':
                            "{% if fields[ 'hr_days' ] %}{{ (fields[ 'hr_days' ]|int)*86400 }}{% else %}0{% endif %}"
                        },
                        'description': {
    ...
    def _SitesHelper__merge_site(self):
        pass

    def update(self):
        site_oper = SiteOper()
        self._sites = site_oper.list_active()
        for i in range(len(self._sites)):
            site = self._sites[i]
            if site.name == '馒头':
                self.sites_indexer['m-team'].update(site.to_dict())
                self.sites_indexer['m-team']['id'] = site.id
                self.sites_indexer['m-team']['domain'] = site.url
                self.sites_indexer['m-team'][
                ...
                self.sites_active.append(self.sites_indexer['m-team'])

            if site.name == '彩虹岛':

                self.sites_indexer['ptchdbits'].update(site.to_dict())
                self.sites_indexer['ptchdbits']['id'] = site.id
                self.sites_indexer['ptchdbits']['domain'] = site.url
                self.sites_indexer['ptchdbits'][
                    'ua'] = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36 Edg/113.0.1774.57'
                self.sites_active.append(self.sites_indexer['m-ptchdbits'])

    def check(self, domain):
        return False, '站点存在'

    def get_indexers(self):
        return self.sites_active

    def get_indexer(self, domain: str):

        dom = domain.split('.')[0]
        self.update()
        for site in self.sites_indexer.keys():
            if dom in site:
                return self.sites_indexer[site]
...
if __name__ == '__main__':
    ...
    def import_sites_from_csv(csv_path: str) -> tuple[int, int, str]:
        """
        从CSV文件导入站点信息
        :return: Tuple[成功数, 失败数, 错误信息]
        """
        import csv
        site_oper = SiteOper()
                ...
                for row in reader:
                    try:
                        site_params = {
                            ...
                            "ua": row.get("ua", None),
                            "apikey": row.get("apikey", None),
                            "token": row.get("token", None)
                        }

                        success, msg = site_oper.add(**site_params)
                        if success:
                            success_count += 1
                        else:
                            failed_count += 1
                            error_msg.append(f"{row.get('domain')}: {msg}")

                    except Exception as e:
                        failed_count += 1
                        error_msg.append(
                            f"{row.get('domain', '未知')}: {str(e)}")
    ...
    csv_path = os.path.join(current_dir, "pt.csv")

    # 导入站点
    success, failed, error_msg = import_sites_from_csv(csv_path)
    # 打印结果
